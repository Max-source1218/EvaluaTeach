...
export default function TabulationSchoolYears({
  fetchUrl,
  token,
  headerTitle,
  headerSubtitle,
  ...
  useEffect(() => {
    console.log('=== TabulationSchoolYears mounted ===');
    ...
    const fetch = async () => {
      try {
        ...
        console.log('=== TabulationSchoolYears data ===');
        console.log('raw data:', JSON.stringify(data));
        if (!cancelled) {
          const normalized = Array.isArray(data)
            ? data.map(item => (typeof item === 'string' ? item : item.schoolyear))
            : [];
          console.log('normalized:', normalized);
          setSchoolYears(normalized);
        }
      } catch (err) {
        console.error('=== TabulationSchoolYears fetch error ===');
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <TouchableOpacity
            style={styles.card}
            onPress={() => {
              console.log('=== School year pressed ===');
              console.log('item:', item);
              console.log('navigateTo:', navigateTo);
              console.log('params:', JSON.stringify(buildParams(item)));
              router.push({ pathname: navigateTo, params: buildParams(item) });
            }}
          >
            ...
            <Text style={styles.cardSubtitle}>View semesters</Text>
        ...
        ListEmptyComponent={
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptySubtext}>No evaluation records exist yet.</Text>
          </View>
        }
      />
    </View>
