...
// ✅ Only 2 sections — Students and Supervisors evaluate Program Chairs
const SECTIONS_CONFIG = [
  { key: 'Student',    title: '👨‍🎓 Student Evaluations',    },
  { key: 'Supervisor', title: '👔 Supervisor Evaluations',  },
];

export default function ChairEvaluationResults() {
  const {
    userId, chairDepartment, userType,
    schoolyear, evaluatorDept, semester, subject,
  } = useLocalSearchParams(); // ✅ all renamed params
  ...
  useEffect(() => {
    if (!userId || !schoolyear || !evaluatorDept || !semester || !subject || !token) {
    ...
    const fetchEvaluations = async () => {
      try {
        const url = `${API_URL}/program-chair-results/results/${userId}/${schoolyear}/${evaluatorDept}/${semester}/${encodeURIComponent(subject)}`;
  ...
  }, [userId, schoolyear, evaluatorDept, semester, subject, token]);

  // ✅ useMemo — only recalculates when evaluations change
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.infoContainer}>
        ...
        <View style={styles.infoRow}>
          <Text style={styles.infoLabel}>Evaluator Dept:</Text>
          <Text style={styles.infoValue}>{evaluatorDept}</Text>
        </View>
      </View>
